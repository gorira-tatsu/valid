...
pub fn testgen_bundled_model(
    ...
    Ok(TestgenResponse {
        ...
        vectors: vectors
            .iter()
            .map(|vector| crate::api::summarize_testgen_vector(vector, 0))
            .collect(),
...
fn build_inspect_response<M: crate::modeling::VerifiedMachine>(
    ...
    InspectResponse {
        ...
        capabilities: InspectCapabilities {
            ...
            parse: capabilities.parse.clone(),
            explicit_ready: capabilities.explicit_ready,
            explicit: capabilities.explicit.clone(),
            ir_ready: capabilities.ir_ready,
            ir: capabilities.ir.clone(),
            solver_ready: capabilities.solver_ready,
            solver: capabilities.solver.clone(),
            coverage_ready: capabilities.coverage_ready,
            coverage: capabilities.coverage.clone(),
