...
- 関連ID:
  ...
  - 要求カタログ: [../11_requirements_catalog/requirements_catalog.md](../11_requirements_catalog/requirements_catalog.md)
  - ガバナンス: [governance_and_operations.md](governance_and_operations.md)
...
- 証拠は `Evidence::Trace` と `Evidence::Certificate` の総称として扱う。
- v0.4 時点では、本番ゲートで使う `FAIL` 証拠は `Evidence::Trace` を必須とする。
...
- v0.4 以降の正規経路では `Rust macro` または `Rust trait` を SSOT とする。
- `spec block` は移行期互換または研究用入力としてのみ扱い、主要経路にしてはならない。
- `Rust macro` と補助入力形式の混在は原則禁止とし、必要な場合は ADR を必須とする。
...
- `schema_version`, `request_id`, `diagnostics` を省略してはならない。
- `seed` は deterministic run では `null` を許可し、その場合も key 自体は省略してはならない。
- `request_id` は外部要求の相関 ID、`run_id` は個別実行と artifact の識別子とする。
...
- backend の出力は共通の result 形式へ正規化しなければならない。
- `FAIL` の証拠は v0.4 では `Evidence::Trace` 必須とする。
- trace を直接出せない backend は `Evidence::Certificate` 対応後まで本番 `FAIL` ゲート対象外とする。
...
`ERROR` は検証行為自体が成立していないことを示し、`assurance_level` は原則 `incomplete` とする。
...
- Rust macro / trait により登録されたモデルが主要経路である。
- `.valid` や `spec block` などの補助入力は暫定ハーネスであり、最終的な主要経路にしてはならない。
...
- 改訂時は [governance_and_operations.md](governance_and_operations.md) と整合していること

## 13. 参照先

- アーキテクチャ詳細: [../03_architecture/architecture.md](../03_architecture/architecture.md)
- ドメイン詳細: [../04_domain/business_logic_and_data_model.md](../04_domain/business_logic_and_data_model.md)
- 機能要件: [../02_requirements/functional_requirements.md](../02_requirements/functional_requirements.md)
- 非機能要件: [../02_requirements/non_functional_requirements.md](../02_requirements/non_functional_requirements.md)
- ガバナンスと運用: [governance_and_operations.md](governance_and_operations.md)
