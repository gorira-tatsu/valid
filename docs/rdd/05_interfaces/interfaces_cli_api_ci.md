...
I/F層は派生物の生成しか行わない。たとえば `doc` は一次ソースを書き換えず、`contract --update` であっても一次ソースではなくlock派生物のみを更新する。この制約を守ることで、入口ごとの差異がSTO違反に繋がることを防ぐ。
...
本章は、利用者ごとに見せ方を変えつつ、意味を変えないことを要求している。CLI、API、CIの一貫性が失われると、同じrunに対して異なる世界が発生するため、この章の契約は厳密に維持する。

## 42. 章末追記

I/Fはもっとも変えやすく見えて、実は最も変更コストが高い。AIクライアント、CI設定、人間運用手順のすべてが依存するためである。そのため、本章の契約は後方互換を最優先とする。

## 43. 付記

本章の契約を破る変更は、常に移行手順を伴う。移行手順が書けない変更は、設計として未熟とみなす。

## 44. 最終注記

本章は利用者境界の仕様書である。したがって、内部実装変更よりも慎重に更新し、互換性を最優先に扱う。

## 45. 終章メモ

安定したI/Fがなければ、自動化は積み上がらない。本章はその土台を固定する。

終。

補足: 本章は利用面の契約を固定する。
補足: 変更時は必ず互換性確認を行う。
補足: CLI、API、CIは同一意味論に従う。
