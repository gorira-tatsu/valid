...
`--backend` は `explicit | mock-bmc | command` を受け付ける。`command` の場合は `--solver-exec` を必須とし、`--solver-arg` は複数回指定できる。

MVP では file path 入力も許可するが、長期的な正規経路は `crate + model_id` による Rust-native model discovery とする。

現時点では、Rust-native の現実例を素早く試すために `valid native-demo <demo-id>` を持つ。対象は次を含む。
