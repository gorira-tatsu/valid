...
### 15.1 SMT backend adoption milestone

関連ID: `FR-023`, `I-1`〜`I-4`, `PR-10`, `PR-12`, `Phase 4`

目的:

- command backend から実 solver adapter へ昇格する
- SMT-COMP 系 solver の比較導入を計画化する
- solver 固有差分を capability matrix と evidence normalization に閉じ込める

方針:

- 単一の「最強 solver」を固定しない
- 最初の正式導入候補は `cvc5` とする
- bit-vector 寄りの強化候補として `Bitwuzla` を評価する
- 互換性と運用知見の観点で `Z3` を比較対象に残す

採用理由:

- SMT-COMP の結果は logic/track ごとに勝者が分かれるため、全 logic で 1 本に固定する運用は避けるべきである
- MVP/Phase 4 では DSL と IR がまだ狭いため、最初は汎用性の高い solver を 1 本正式導入し、その後 benchmark suite ベースで増やす

完了条件:

- `command` backend ではなく `smt-cvc5` の専用 adapter が動く
- adapter は `PASS/FAIL/UNKNOWN` を共通 result 形式へ正規化できる
- solver 導入の acceptance benchmark が CI で回る
- `mock-bmc` ではなく実 solver で witness/counterexample が 1 モデル以上で再生できる

移行順序:

1. benchmark suite を固定する
2. `cvc5` adapter を導入する
3. `Bitwuzla` を bit-vector 用候補として評価する
4. solver 選定結果を ADR と capability matrix へ反映する
