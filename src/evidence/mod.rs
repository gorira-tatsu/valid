...
use crate::{
    ...
    ir::Value,
    support::{artifact::{evidence_path, run_result_path, vector_path}, diagnostics::Diagnostic, io::write_text_file},
};
...
pub fn write_outcome_artifacts(model_id: &str, outcome: &CheckOutcome) -> Result<Vec<String>, String> {
...
pub fn render_trace_json(trace: &EvidenceTrace) -> String {
    ...
    for (index, step) in trace.steps.iter().enumerate() {
        ...
        if let Some(action_id) = &step.action_id { out.push_str(&format!(",\"action_id\":\"{}\"", action_id)); } else { out.push_str(",\"action_id\":null"); }
        if let Some(action_label) = &step.action_label { out.push_str(&format!(",\"action_label\":\"{}\"", action_label)); } else { out.push_str(",\"action_label\":null"); }
...
fn render_completed_json(model_id: &str, result: &ExplicitRunResult) -> String {
    ...
    if let Some(reason_code) = &result.property_result.reason_code { out.push_str(&format!(",\"reason_code\":\"{}\"", escape_json(reason_code))); } else { out.push_str(",\"reason_code\":null"); }
    if let Some(reason) = result.property_result.unknown_reason { out.push_str(&format!(",\"unknown_reason\":\"{}\"", unknown_reason_label(reason))); } else { out.push_str(",\"unknown_reason\":null"); }
    if let Some(terminal_state_id) = &result.property_result.terminal_state_id { out.push_str(&format!(",\"terminal_state_id\":\"{}\"", escape_json(terminal_state_id))); } else { out.push_str(",\"terminal_state_id\":null"); }
    if let Some(evidence_id) = &result.property_result.evidence_id { out.push_str(&format!(",\"evidence_id\":\"{}\"", escape_json(evidence_id))); } else { out.push_str(",\"evidence_id\":null"); }
    out.push_str(&format!(",\"summary\":\"{}\"", escape_json(&result.property_result.summary)));
    out.push('}');
    if let Some(trace) = &result.trace { out.push_str(&format!(",\"trace\":{}", render_trace_json(trace))); } else { out.push_str(",\"trace\":null"); }
    out.push('}');
    out
}
...
fn append_manifest(out: &mut String, manifest: &crate::engine::RunManifest) {
    ...
    if let Some(seed) = manifest.seed { out.push_str(&format!(",\"seed\":{}", seed)); } else { out.push_str(",\"seed\":null"); }
    out.push('}');
}
...
fn status_label(status: RunStatus) -> &'static str { match status { RunStatus::Pass => "PASS", RunStatus::Fail => "FAIL", RunStatus::Unknown => "UNKNOWN" } }
fn error_status_label(status: ErrorStatus) -> &'static str { match status { ErrorStatus::Error => "ERROR" } }
fn property_kind_label(kind: &crate::ir::PropertyKind) -> &'static str { match kind { crate::ir::PropertyKind::Invariant => "invariant" } }
fn assurance_label(level: AssuranceLevel) -> &'static str { match level { AssuranceLevel::Complete => "complete", AssuranceLevel::Bounded => "bounded", AssuranceLevel::Incomplete => "incomplete" } }
fn unknown_reason_label(reason: UnknownReason) -> &'static str { match reason { UnknownReason::StateLimitReached => "UNKNOWN_STATE_LIMIT_REACHED", UnknownReason::TimeLimitReached => "UNKNOWN_TIME_LIMIT_REACHED", UnknownReason::EngineAborted => "UNKNOWN_ENGINE_ABORTED" } }
fn backend_label(kind: crate::engine::BackendKind) -> &'static str { match kind { crate::engine::BackendKind::Explicit => "explicit" } }
fn evidence_kind_label(kind: &EvidenceKind) -> &'static str { match kind { EvidenceKind::Trace => "trace", EvidenceKind::Certificate => "certificate" } }
fn value_json(value: &Value) -> String { match value { Value::Bool(value) => value.to_string(), Value::UInt(value) => value.to_string() } }
fn escape_json(input: &str) -> String { input.replace('\\', "\\\\").replace('"', "\\\"").replace('\n', "\\n") }
...
mod tests {
    ...
    use super::{render_outcome_json, write_outcome_artifacts, EvidenceKind, EvidenceTrace, TraceStep};
    ...
    fn writes_artifacts_for_completed_outcome() {
        ...
        let paths = write_outcome_artifacts("Counter", &CheckOutcome::Completed(result)).unwrap();
