...
pub fn evaluate_guard(model: &ModelIr, state: &MachineState, action: &ActionIr) -> Result<bool, Diagnostic> {
    match eval_expr(model, state, &action.guard)? {
        Value::Bool(value) => Ok(value),
        _ => Err(Diagnostic::new(
            ErrorCode::EvalError,
            DiagnosticSegment::KernelEval,
            format!("guard for action `{}` did not evaluate to bool", action.action_id),
        )
